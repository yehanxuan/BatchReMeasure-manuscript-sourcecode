# plot the detection of the significant genes (S8)#
rm(list = ls())
...
create.data.discover.sig = function(res.pv.a, Sig_Ind, procedure = "BH") {
  res.df <- reshape::melt(res.pv.a)
  colnames(res.df)[ncol(res.df)] <- 'Value'
  if (procedure == "BH") {
    m <- aggregate(Value~Method + ReMeasureNo, res.df, function(x) {
     Rej_Ind =  (p.adjust(x, method = "BH") < alpha) == TRUE
     sum( which(Rej_Ind) %in% Sig_Ind )
    } ) 
  } else if (procedure == 'bonferroni') {
    m <- aggregate(Value~Method + ReMeasureNo, res.df, function(x) {
      Rej_Ind =  (p.adjust(x, method = "bonferroni") < alpha) == TRUE
      sum( which(Rej_Ind) %in% Sig_Ind )
    } ) 
  }
...
for (l in 1:length(DList)) {
  ...
  for (m in 1:length(allMethods)) {
    method = allMethods[m]
    result = read.csv(paste0( './data/', paste(Datalist, collapse = '-'), "-method-", method,
  ...
  res.df = create.data.discover.sig(res.pv.a,Sig_Ind, procedure)
...
GraphName = paste0("./figure/FigS8.pdf")
